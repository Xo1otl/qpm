...
カスケード第三高調波発生（Cascaded Third Harmonic Generation, cTHG）は，二次の非線形光学効果を多段に利用することで，基本波から第三高調波を生成する現象である．
...
カスケード第三高調波発生における基本波 ($A_1$)，第二高調波 ($A_2$)，第三高調波 ($A_3$) の複素振幅の空間的発展は，SHGとSFGの結合波方程式を連立させることで記述される．
Suharaらの結合モード理論に基づき，前節までに定義した結合係数 $\kappa_{\text{SHG}}, \kappa_{\text{SFG}}$ および位相不整合量 $\Delta k_{\text{SHG}}, \Delta k_{\text{SFG}}$ を用いると，cTHGの結合波方程式は以下のようになる．
...
変換効率が十分に低い場合，基本波の枯渇を無視する非励起減衰近似（NPDA）を適用することができる．
...
と表せる．
この式は，相互作用係数 $J_{nm}$ を持つスピン系（イジングモデル）のハミルトニアン（エネルギー関数）と同様の形式（二次形式）をしている．
したがって，cTHGの変換効率最大化（$|S|$ の最大化）問題は，相互作用行列 $J$ を持つ一般化イジングモデルの基底状態（あるいは最大エネルギー状態）探索問題に帰着することができる．
一般に，このような相互作用を含むイジングモデルの基底状態探索はNP困難な問題として知られている．
そのため，通常の計算機では厳密解を得ることが困難であるが，近年では量子アニーリングなどの手法がこの種の問題に対して有効であると期待されている．
